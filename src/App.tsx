  ...
  useEffect(() => {
    if (accounts.length > 0) {
      const pullAccountInfo = async () => {
        ...
        fetch(`https://api.blockcypher.com/v1/btc/${await getNetwork() === 'livenet' ? 'main' : 'test3'}/addrs/${accounts[0]}/balance`).then(r => r.json()).then(data => setBalanceBTC(data.balance / 1e8));
  ...
  return (
    <div className="App">
      <div className="logo-section">
        ...
        <img src="https://i.imgur.com/I3rmeOX.png" alt="Logo 2" className="logo logo-big-b2" />
      </div>
      {!address ? (
      <button className="sign-button" onClick={handleLogin}>
        ...
        <div className="profile-card">
          ...
          <button className="sign-message-button" onClick={executeTxBtc}>Execute BTC Transaction</button>
